...
export function ChatPanel({ open, onClose, onOpenSettings, onRefresh }: ChatPanelProps) {
  ...
  useEffect(() => {
    invoke<boolean>("has_claude_api_key").then(setHasApiKey).catch(() => {});
  }, [open]);
  ...
  useEffect(() => {
    ...
    listen<string>("chat:tool_call", (event) => {
      setStreaming((prev) => prev + `\n[Calling ${event.payload}...]`);
    }).then((unlisten) => unlisteners.push(unlisten));
    ...
    listen<boolean>("chat:complete", () => {
      ...
      invoke<ChatEntry[]>("get_chat_history").then(setMessages).catch(() => {});
      onRefresh();
    }).then((unlisten) => unlisteners.push(unlisten));
        ...
        {!hasApiKey && (
          <div className="bg-surface-2 border-border rounded border p-3 text-center">
            ...
            <p className="text-text-2 mb-2 text-xs">
              Set your Claude API key in Settings to use the chat.
            </p>
            <button
        ...
        {messages.map((msg, i) => (
          <div key={i} className={`text-xs ${msg.role === "user" ? "text-right" : ""}`}>
            {msg.role === "user" ? (
              ...
              <div className="text-text-2 flex items-center gap-1 py-0.5 text-[10px] italic">
      ...
      <div className="border-border border-t p-2">
        <div className="flex gap-1.5">
          <input
            ...
            placeholder={hasApiKey ? "Ask Claude to edit your show..." : "API key required"}
