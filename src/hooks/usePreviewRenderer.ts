...
import type { CameraState } from "./usePreviewCamera";

const BG_COLOR = "#0E0E0E";
const BASE_RADIUS = 3;
const GLOW_SCALE = 4; // 1/4 resolution for glow layer
const DEFAULT_GLOW_ALPHA = 0.12;
...
function renderFrame(
  ...
  const bulbSize = settings?.bulbSize ?? 1;
  const bulbOpacity = settings?.bulbOpacity ?? 1;
  const glowSize = settings?.glowSize ?? 1;
  const glowAlpha = settings?.glowOpacity ?? DEFAULT_GLOW_ALPHA;
  ...
  // 4. Glow layer at 1/4 resolution (additive)
  if (glowAlpha > 0 && glowSize > 0) {
    const w = geo.width;
    const h = geo.height;
    ...
    if (glowCtx) {
      ...
      const glowRadius = Math.max(2, Math.round(BASE_RADIUS * 1.5 * glowSize / GLOW_SCALE));
      ...
      // 5. Composite glow additively (scaled up with smoothing)
      ctx.save();
      ctx.globalCompositeOperation = "lighter";
      ctx.imageSmoothingEnabled = true;
      // Apply camera transform for glow
      ctx.translate(panX * dpr, panY * dpr);
      ctx.scale(zoom, zoom);
      ctx.drawImage(glowCanvas, 0, 0, glowW, glowH, 0, 0, geo.width * dpr, geo.height * dpr);
      ctx.restore();
    }
  }

  // 6. Batched color circles -- additive blending, no borders
  ctx.save();
  ctx.globalCompositeOperation = "lighter";
  ctx.scale(dpr, dpr);
...
export function usePreviewRenderer(
  ...
  const prevShowRef = useRef<Show | null>(null);
  const prevWidthRef = useRef(0);
  const prevHeightRef = useRef(0);

  // Single effect: recompute geometry when needed, then render
  useEffect(() => {
    const canvas = canvasRef.current;
    if (!canvas || !show || width <= 0 || height <= 0) return;

    // Recompute geometry if show or dimensions changed
    if (show !== prevShowRef.current || width !== prevWidthRef.current || height !== prevHeightRef.current) {
    ...
    if (!geo || !colorBuf) return;

    const dpr = window.devicePixelRatio || 1;
    canvas.width = geo.width * dpr;
    canvas.height = geo.height * dpr;

    renderFrame(canvas, geo, frame, colorBuf, dpr, settings, camera);
  }, [canvasRef, frame, width, height, show, settings, camera]);
}
